...
namespace Rido.IoTClient.AzBroker.TopicBindings
{
    public class WritableProperty<T>
    {
        ...
        public async Task UpdateTwinAsync() => await updateTwin.UpdatePropertyAsync(this.PropertyValue.ToAckDict());
        ...
        public async Task InitPropertyAsync(string twin, T defaultValue, CancellationToken cancellationToken = default)
        {
            ...
            if (desiredBinder.OnProperty_Updated != null && (PropertyValue.DesiredVersion > 1))
            {
                ...
                _ = updateTwin.UpdatePropertyAsync(ack.ToAckDict(), cancellationToken);
                PropertyValue = ack;
            }
            else
            {
                _ = updateTwin.UpdatePropertyAsync(PropertyValue.ToAckDict());
            }
        }
