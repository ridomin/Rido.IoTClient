...
namespace pnp_aws_sample
{
    public class DeviceRunner : BackgroundService
    {
        ...
        async Task<dtmi_rido_pnp.Cmd_getRuntimeStats_Response> Command_getRuntimeStats_Handler(dtmi_rido_pnp.Cmd_getRuntimeStats_Request req)
        {
            commandCounter++;
            var result = new dtmi_rido_pnp.Cmd_getRuntimeStats_Response()
            {
                Status = 200
            };
            ...
            if (req.DiagnosticsMode == dtmi_rido_pnp.DiagnosticsMode.complete)
            {
                result.diagnosticResults.Add("this app:", System.Reflection.Assembly.GetExecutingAssembly()?.FullName ?? "");
            }
            if (req.DiagnosticsMode == dtmi_rido_pnp.DiagnosticsMode.full)
            {
